...
* ```toggle``` : 对整个元素的可见样式属性进行动画过渡处理(宽度```width``` 高度```height``` 透明度```opacity```) -- 既是 切换 显示与隐藏 效果
...
## 动画效果
**animate()**
* 该方法通过 ```CSS ```样式将元素从一个状态改变为 另一个状态```.CSS```属性值是逐渐改变的,这样就可以创建动画效果
...
* 语法 : 
    ...
    *``` .animate( properties , options )```
        ...
        * 可能的值为 : 
            ...
            * ```queue  ```   --      布尔值.指示是否在效果队列中放置动画.如果为```false ```,则动画立即开始
...
## jQuery 核心
**each()**
* 大部分``` jQuery ```方法在内部都会调用 ```each() ```方法,其主要原因就是``` jQuery ```的实例是一个元素合集
* ```$('div').css(....)```
* 找到所有的``` div ```,并且都设置样式, ```css``` 只是一个方法,所以内部会调用 ```each``` 处理这个 ```div ```合集,给每个 ```div ```都设置``` style``` 属性
* ```.each()``` 方法只是处理``` jQuery```对象的方法, ```jQuery ```还提供了一个通用的 ```jQuery.each()``` (或者直接写成 ```$.each() ```) 方法,用来处理对象和数组的遍历
    * ```$.each(array,callback)```
    * ```$each(object,callback)```
    * callback 中的回调函数为 : ```function (index, value)```
        * 如果是 ```array```, 则 ```index``` 表示数组的索引(从 ```0``` 开始)
        * 如果是 ```object```, 则``` index``` 表示对象``` k: v```键值对中的``` k```
* ```jQuery.each() ``` (或写成``` $.each() ```) 函数还会根据每次调用函数 ```callback ```的返回值来决定后续动作.如果返回值为 ```false``` ,则停止循环(相当于普通循环中的 ```break``` ) ;如果返回其他值,则都表示继续执行下一个循环

**inArray**
* 在 ```ECMAScript5 ```中已经有数组的``` indexOf() ```方法支持了,但是 ```jQuery ```保持了版本向下兼容,所以封装了一个 ```inArray``` 方法
* ```jQuery.inArray()```函数用于在数组中搜索指定的值, 并返回其索引值.如果数组中不存在该值,则返回``` -1```
* ```jQuery.inArray( value, array ,[ fromIndex ] )```
    * ```fromIndex``` 可选, 默认值为``` 0 ```
* **注意** : 如果要判断数组中是否存在指定值, 则需要通过该函数的返回值是否为``` -1 ```来进行判断

**trim**
* 存在的意义 : 常见的登录信息的提交处理.用户的输入不一定是标准的,输入一段密码：' ``` 1123456 ``` ',注意了： 密码的前后会留空,这可能是用户的无心的行为,但是密码确实又没错,针对这样的行为,开发者应该要判断输入值的前后是否有空白符、换行符、制表符这样明显的无意义的输入值.
...
**get**
* 用 ```.get() ```方法获取``` jQuery``` 合集中的某一个元素
* 语法 : ```.get( [ index ] )```
    * ```index ```为正值时,索引从 ```0``` 开始,从前往后找
    *``` index``` 为负值时,索引从 ```-1 ```开始,从后往前找
* ```.get() ```方法获取的是 ```DOM``` 对象,也就是通过``` document.getElementById()``` 获取的对象
* ```.get()``` 方法 与 ```:eq()```
    * 如果想将``` .get()``` 改成 ```:eq() ```也是可以的,但并不是等价交换的,他们之间还是有一些差别的
    * 因为``` $("ele").get()```  得到的是``` DOM ```对象,也就是变成了 ```JS``` ,那么 ```JS``` 取得的元素后面就必须跟 ```JS ```的方法或者属性,比如 :```$("ele").get().style()``` 而不能是``` $("ele").get().css()```. 因为 ```style``` 是``` JS``` 独有的方法,而 ```css()``` 是 ```jQuery ```独有的方法, ```.get() ```方法后取得的``` JS ```元素,只能识别``` style()``` 方法,而不能识别``` css()``` 方法

**index**
*  ```get``` 方法是通过已知的索引在合集中找到对应的元素,``` index ```是通过元素确定该元素在合集中对应的索引
* ```.index() ```方法,从匹配元素中搜索给定元素的索引值,从 ```0 ```开始计数
* 语法: 参数接受一个 j```Query``` 或者``` DOM``` 对象作为查找的条件
    * ```.index()```
        * 返回 第一个匹配元素在其同辈元素中的位置
    * ```.index( selector )```
        * 返回 原先元素相对于选择器匹配元素的位置.如果找不到匹配的元素,则 index(selector) 返回 -1
    * ```.index(element )```
        * 如果参数是一个 ```DOM``` 元素,或者 ```jQuery``` 对象,```index( element ) ```返回的就是 传入的元素相对于原先集合的位置
* ```eg```
    ```
    <ul>
        <a></a>
    ...
    ```
    ```
    $("li").index()   // 1  ,因为同辈从前到后,从 a 便签开始算(记为 0 )
    // 如果要快速找到第二个 li 在列表中的索引,可以通过下面两种方式
    $("li").index($("#test2"))  // 1 
